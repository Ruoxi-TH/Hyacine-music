...
export default function HomeScreen(): React.JSX.Element {
  ...
  const loadDailySongs = useCallback(async (): Promise<void> => {
    ...
    if (!profile?.backendUrl || profile.musicSource !== "netease") {
      if (seq === requestSeq.current) {
        setSongs([]);
        setLoading(false);
        setError(t("neteaseRequiredDaily"));
      }
    ...
    try {
      if (!(await supportsNeteaseCapability(profile.backendUrl, "dailySongs"))) {
        if (seq === requestSeq.current) {
          setSongs([]);
          setError(t("neteaseDailyUnsupported"));
        }
      ...
      if (!response.ok || !Array.isArray(data)) throw new Error((data as { message?: string }).message || t("loadDailyFailed"));
      if (seq !== requestSeq.current) {
      ...
      setSongs([]);
      setError(cause instanceof Error ? cause.message : t("loadDailyFailed"));
      appLog.error("home", "daily songs load failed", cause);
    } finally {
      if (seq === requestSeq.current) setLoading(false);
    }
  }, [getSourceCredential, profile?.backendUrl, profile?.musicSource, t]);
  ...
  const featuredPlaybackState = (() => {
    if (!featured) return "idle" as const;
    if (currentTrack?.id === featured.id) return isPlaying ? "playing" : "paused" as const;
    if (pendingId === featured.id) return "pending" as const;
    return "idle" as const;
  })();
  const featuredLabel = featuredPlaybackState === "playing"
    ? t("nowPlayingShort")
    : featuredPlaybackState === "pending"
      ? t("playPending")
      : t("playRecommend");
  ...
  return <ThemedScreen>
    <ScrollView contentContainerClassName="px-5 pb-40 pt-16">
      <View className="flex-row items-start justify-between">
        <View><Text style={{ color: tokens.text, fontSize: 31, fontWeight: "800" }}>{t("greeting")}</Text><Text className="mt-2 text-sm" style={{ color: tokens.mutedText }}>{t("dailyRecommend")}</Text></View>
        ...
        <Pressable className="overflow-hidden p-5" onPress={() => void onPlay(featured)}>
          ...
          <View className="min-h-48 flex-row items-end">
            <View className="min-w-0 flex-1 pr-4">
              <Text className="text-xs font-bold" style={{ color: tokens.accent }}>{t("featuredBadge")}</Text>
          ...
          <LiquidControlSurface className="mt-5 h-12 self-start rounded-full px-5" style={{ borderRadius: 24 }}><View className="h-full flex-row items-center justify-center"><Text style={{ color: tokens.text, fontWeight: "800" }}>{featuredLabel}</Text></View></LiquidControlSurface>
        </Pressable>
      </ThemedCard> : null}
      ...
      <View className="mt-4 gap-3">{songs.slice(1, 9).map((song) => {
        const songState = currentTrack?.id === song.id ? "playing" : pendingId === song.id ? "pending" : "idle";
        return <ThemedCard key={song.id} className="p-0" style={{ borderRadius: 20 }}><Pressable className="flex-row items-center p-3" style={{ backgroundColor: "transparent" }} onPress={() => void onPlay(song)}>{song.artwork ? <Image className="h-14 w-14 rounded-2xl" source={{ uri: song.artwork }} contentFit="cover" /> : <View className="h-14 w-14 items-center justify-center rounded-2xl" style={{ backgroundColor: `${tokens.accent}18` }}><Text style={{ color: tokens.accent, fontWeight: "900" }}>♪</Text></View>}<View className="ml-3 min-w-0 flex-1"><Text numberOfLines={1} style={{ color: tokens.text, fontWeight: "800" }}>{song.title}</Text><Text className="mt-1 text-xs" numberOfLines={1} style={{ color: tokens.mutedText }}>{song.artist}</Text></View><Text style={{ color: tokens.accent, fontSize: 18 }}>{songState === "playing" ? "Ⅱ" : songState === "pending" ? "…" : "▶"}</Text></Pressable></ThemedCard>;
      })}</View>
    </ScrollView>
  </ThemedScreen>;
}